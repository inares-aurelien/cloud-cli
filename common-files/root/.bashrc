...
for x in /usr/local/sbin /usr/local/bin /usr/sbin /usr/bin /sbin /bin $HOME/bin $HOME/.local/bin; do
  case ":$PATH:" in
    *":$x:"*) :;; # already there
...
duargs='-h'
dusargs='-x'
if [[ "$duhelp" == *"max-depth"* ]] ; then
  duargs+=' --max-depth='
fi
if [[ "$duhelp" == *"block-size"* ]] ; then
  dusargs+=' --block-size=1048576'   # Affichage en Mo
fi
if [[ "$duhelp" == *"BusyBox"* ]] ; then
  duargs+=' -d '
fi
...
if [[ "$lshelp" == *"time-style"* ]] ; then
  lsargs+=' --time-style=+%d/%m/%Y_%H:%M'
fi
alias ll='ls $lsargs'
unset lshelp
...
if has "az"; then
  ...
  echo -e "Azure-CLI (python) is installed\n"
fi

if has "azure"; then
  eval $(azure --completion)
  echo -e "Azure-CLI (nodeJS) is installed\n"
fi

if has "aws"; then
  echo -e "AWS-CLI (python) is installed\n"
fi
